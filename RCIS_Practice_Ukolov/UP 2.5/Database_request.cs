...
namespace GarageConsoleApp
{
    ...
    public static class DatabaseRequests
    {
        ...
        public static void AddCarQuery(int idTypeCar, string name, string stateNumber, int numberPassangers)
        {
            var querySql = $"INSERT INTO car(id_type_car, name, state_number, number_passengers) VALUES ({idTypeCar}, '{name}', '{stateNumber}', {numberPassangers})";
            using var cmd = new NpgsqlCommand(querySql, DatabaseService.GetSqlConnection());
            cmd.ExecuteNonQuery();
        }
        ...
        public static void GetRouteQuery()
        {
            ...
            while (reader.Read())
            {
                Console.WriteLine($"ID: {reader[0]} ID водителя: {reader[1]} ID машины: {reader[2]}, ID маршрута: {reader[3]} Кол-во пассажиров: {reader[4]}");
            }
        }
    }
}